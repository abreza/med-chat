...
class PiperEngine:
    ...
    def check_voice_files_exist(self, voice_key: str) -> bool:
    ...
    def load_voice_model(self, voice_key: str) -> Optional[Any]:
        ...
        try:
            ...
            model_path, config_path = self.get_voice_file_paths(voice_key)
            if not model_path or not config_path:
                print(f"Voice {voice_key} not found in configuration")
                return None

            if not self.check_voice_files_exist(voice_key):
                print(f"Voice files for {voice_key} not found locally")
                print(f"Expected files: {model_path}, {config_path}")
                return None
