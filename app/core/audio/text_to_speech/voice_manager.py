...
class VoiceManager:
    ...
    def _load_voices(self) -> Dict[str, Any]:
        ...
        if not voices_file.exists():
            print(f"Warning: voices.json not found at {voices_file}")
            return {}
        ...
        try:
            ...
            print(f"Loaded {len(processed_voices)} Piper voices from voices.json")
    ...
    def _process_voice_info(self, voice_key: str, voice_info: Dict[str, Any], 
                          onnx_file: str, config_file: str) -> Dict[str, Any]:
    ...
    def get_speaker_count(self, voice_key: str) -> int:
        if voice_key in self.voices:
            return self.voices[voice_key].get("num_speakers", 1)
        return 1

    def get_speaker_names(self, voice_key: str) -> List[str]:
        if voice_key in self.voices:
            speaker_map = self.voices[voice_key].get("speaker_id_map", {})
            if speaker_map:
        ...
        for key, value in kwargs.items():
            if key in self.current_settings and value is not None:
                if key == "voice" and value in self.voices:
                    ...
                    max_speakers = self.get_speaker_count(self.current_settings["voice"])
                    self.current_settings[key] = max(0, min(int(value), max_speakers - 1))
                elif key == "speed":
                    self.current_settings[key] = max(0.1, min(3.0, float(value)))
                elif key in ["noise_scale", "noise_scale_w"]:
                    self.current_settings[key] = max(0.0, min(1.0, float(value)))
