...
class AudioHandlers:
    ...
    def generate_speech_audio(self, text: str, ai_message: str = None) -> Optional[str]:
        ...
        try:
            ...
            if not current_voice or not self.voice_manager.validate_voice(current_voice):
                print("No valid voice available for TTS")
                ...
                if voice_options:
                    ...
                    print(f"Using fallback voice: {current_voice}")
                else:
    ...
    def test_voice_settings(self, voice_key: str, speaker_id: int, speed: float,
                          noise_scale: float, noise_scale_w: float) -> Optional[str]:
